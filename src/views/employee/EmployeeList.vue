...
function handleView(rowData) {
    ...
    router.push({ path: '/employee/detail', query: { employeeId: rowData.loginId } });
}
...
const loadData = async () => {
    ...
    try {
        ...
        // API 호출
        const response = await $api.member.getParams('search/list', queryString);
...
const exportCSV = async () => {
    loading.value = true;
    try {
        const blob = await $api.member.get('excel', '', {
            responseType: 'blob'
        });
        ...
        link.href = url;
        link.setAttribute('download', 'employeeExcel.xlsx');
